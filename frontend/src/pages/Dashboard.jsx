...
const Dashboard = () => {
  ...
  const handleAddWearable = async () => {
    ...
    try {
      let type = "shirt";
      const inputLower = wearableInput.toLowerCase();

      if (inputLower.includes('dress') || inputLower.includes('gown')) {
        type = "dress";
      } else if (inputLower.includes('pant') || inputLower.includes('jean') || inputLower.includes('trouser')) {
        type = "pants";
      }

      // Generate a dynamic name just for display
      const assignedName = `Garment ${(avatar?.wearables?.length || 0) + 1}`;

      const result = await api.addWearableFromUrl(
        wearableInput,
        assignedName,
        type
      );
      ...
      console.error(err);
      alert('Failed to generate 3D wearable from URL');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitPhoto = async () => {
    ...
    try {
      ...
      console.log("PIFuHD result:", result);

      if (result.success && result.modelUrl) {
        await loadAvatar(); // Fetch the newly created or updated avatar from DB
        alert("3D model generated successfully! Switching to Dashboard view...");
        setActiveTab('home');
      } else {
        alert("Model generation started but no URL returned");
      }
    } catch (err) {
      console.error(err);
          ...
          {[
            ...
            return (
              <button
                ...
                className={`w-full flex items-center ${sidebarOpen ? 'space-x-4' : 'justify-center'} px-4 py-3 rounded-xl transition-all duration-300 group ${activeTab === item.id
        ...
        {activeTab === 'avatar' && (
          <div className="p-8 animate-fade-in">
            <div className="grid lg:grid-cols-2 gap-8">
              {/* Photo Upload Section or 3D Preview */}
              <div className="space-y-6">
                {avatar?.modelUrl ? (
                  <div className="relative group">
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-600/30 to-purple-600/30 rounded-2xl blur-3xl group-hover:blur-3xl transition-all duration-500" />
                    <div className="relative bg-gray-900 border border-gray-800 rounded-2xl p-4 h-[600px] overflow-hidden group-hover:border-indigo-600/50 transition-all duration-300">
                      <h3 className="text-xl font-bold mb-4 absolute top-6 left-6 z-10">3D Preview</h3>
                      <ErrorBoundary>
                        <RealisticAvatar3D
                          measurements={measurements}
                          modelUrl={avatar?.modelUrl}
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm("Are you sure you want to upload a new photo and replace your current 3D model?")) {
                            setAvatar(prev => prev ? { ...prev, modelUrl: null } : null);
                          }
                        }}
                        className="absolute top-6 right-6 z-10 px-3 py-1 bg-gray-800/80 hover:bg-neutral-600/80 rounded text-xs backdrop-blur-md transition-colors"
                      >
                        Upload New Photo
                      </button>
                    </div>
          ...
          <div className="p-8 space-y-8 animate-fade-in">
            {!avatar ? (
              ...
              <>
                ...
                {/* Selected Wearable Preview & Fit Analysis */}
                {selectedWearable && (
                  <div className="grid lg:grid-cols-2 gap-8 mb-8 animate-fade-in relative z-10">
                    {/* 3D Preview */}
                    <div className="relative group">
                      <div className="absolute inset-0 bg-gradient-to-br from-indigo-600/30 to-purple-600/30 rounded-2xl blur-3xl group-hover:blur-3xl transition-all duration-500" />
                      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl p-4 h-[500px] overflow-hidden group-hover:border-indigo-600/50 transition-all duration-300">
                        <h3 className="text-xl font-bold mb-4 absolute top-6 left-6 z-10">Try On: {selectedWearable.name}</h3>
                        <ErrorBoundary>
                          <RealisticAvatar3D
                            measurements={measurements}
                            modelUrl={avatar?.modelUrl}
                        ...
                        <button
                          ...
                          className="absolute top-6 right-6 z-10 px-3 py-1 bg-gray-800/80 hover:bg-red-600/80 rounded text-xs backdrop-blur-md transition-colors"
                        >
                          Close Preview
                        </button>
                      </div>
                    ...
                    {/* Fit Analysis */}
                    <div className="relative group">
                      <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/30 to-blue-600/30 rounded-2xl blur-3xl group-hover:blur-3xl transition-all duration-500" />
                      <div className="relative bg-gradient-to-br from-gray-900 to-gray-950 border border-gray-800 rounded-2xl p-8 group-hover:border-cyan-600/50 transition-all duration-300 h-full flex flex-col">
                        <h3 className="text-2xl font-bold mb-6 flex items-center space-x-2">
                        ...
                        <div className="flex-1 space-y-6">
                          <div className="p-4 bg-gray-800/50 rounded-xl border border-gray-700">
                            <h4 className="font-semibold text-lg text-white mb-2 flex items-center gap-2">
                              <span>📏</span> Measurement Check
                            </h4>
                            <p className="text-gray-400 text-sm">
                              Based on your profile (Chest: {measurements.chest}cm, Waist: {measurements.waist}cm), this item should provide a comfortable, true-to-size fit.
                            </p>
                          </div>

                          <div className="p-4 bg-green-900/20 border border-green-500/30 rounded-xl">
                            <h4 className="font-semibold text-lg text-green-400 mb-2 flex items-center gap-2">
                              <CheckCircle className="w-5 h-5" /> Recommended Size: Medium
                            </h4>
                            <p className="text-gray-300 text-sm">
                              The {selectedWearable.name} matches your shoulder and inseam proportions nicely. The fabric flow will drape well over your generated 3D model.
                            </p>
                          </div>
                        </div>

                        <button
                          className="w-full mt-6 px-6 py-4 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-xl font-medium hover:shadow-lg hover:shadow-cyan-600/50 transition-all duration-300 active:scale-95 flex justify-center items-center gap-2"
                          onClick={() => alert("Added to cart!")}
                        >
                          <Shirt className="w-5 h-5" /> Purchase Items
                        </button>
                      </div>
                    </div>
                  </div>
                            ...
                            <div className="p-4">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${selectedWearable?._id === wearable._id
